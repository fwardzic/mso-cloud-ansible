...
- |
  ...
  and the objects key attribute are defined in module_utils/mso_const.py .
author:
- Daniel Schmidt
version_added: '2.8'
options:
...
from ansible_collections.cisco.aac.plugins.module_utils.mso_const import (
...
def main():
    ...
    def query_objs(path, key=None, **kwargs):
        found = []
    ...
    def lookup(path, search_key):
        ...
        if lookup_cache.get(path) is None:
            lookup_cache[path] = query_objs(path, key=container)
